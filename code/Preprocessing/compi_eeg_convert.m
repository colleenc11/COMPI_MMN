function D = compi_eeg_convert(S)
% -------------------------------------------------------------------------
% COMPI_EEG_CONVERT Adapted from spm_eeg_covert. For the COMPI study, we 
% specifically update the channel labels by removing any occurrences of '1-'.
% 
...
%     hdr.chanunit = cellfun(@(x) strrep(x,'unknown','uV'), hdr.chanunit,  'UniformOutput', false);
    ...
    try
        ...
        event = ft_read_event(S.dataset, 'detectflank', 'both', 'eventformat', 'biosemi_bdf'); %% specific for reading EDF data
