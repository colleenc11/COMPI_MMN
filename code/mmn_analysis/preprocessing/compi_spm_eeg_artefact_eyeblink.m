function res = compi_spm_eeg_artefact_eyeblink(S)
...
% 
% The code, based on spm_eeg_artefact_eyeblink, has been modified as follows:
%   1. We now save the number of eye-blinks per min for quality checking.
%   2. We have changed the line "ev(Nevents+i).duration = []" to 
%      "ev(Nevents+i).duration = 0". This change is made because when the 
%      duration value is an empty array, it leads to compatibility issues 
%      with arrays in subsequent steps of the code.
...
try 
    run = extractBetween(S.D.fname,'Run', '.mat');
    save(['EBperMin_Run' run{1}],'num_eb_per_min');
catch
    save('EBperMin_Run','num_eb_per_min');
end
