function res = tnueeg_spm_eeg_artefact_ampthresh(S)
%TNUEEG_SPM_EEG_ARTEFACT_AMPTHRESH Modified SPM function:
%Detects eyeblinks in SPM continuous data file
% S                     - input structure
% fields of S:
%    S.D                - M/EEG object
%    S.chanind          - vector of indices of channels that this plugin will look at.
%    S.threshold        - threshold parameter (in uV)
%
...
if isempty(spikes)
    warning('No eye-blinks detected by algorithm. Try a lower threshold.')
end
...
if (num_eb_per_min<0.5)
    warning(['Only ' num2str(num_eb_per_min) ' eye-blinks per minute detected by algorithm. Try a lower threshold.'])
end
...
clf
subplot(2, 1 , 1)
plot(spikes,ones(length(spikes),1)*S.threshold,'r.');
hold on;
plot(eog_filt);
