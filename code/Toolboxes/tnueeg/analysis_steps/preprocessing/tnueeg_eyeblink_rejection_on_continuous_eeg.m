...
rejectionMode   = options.eeg.preproc.eyeblinkmode;     % 'eventbased', 'timebased', 
EBwindow        = 1%options.eeg.preproc.eyeblinkwin;
ERPlength       = options.eeg.preproc.epochwin(2)/1000; % end of ERP window in s
ERPoffset       = options.eeg.preproc.eyeblinktrialoffset; % in s: window to discard at beg. of trial
EOGchannel      = options.eeg.preproc.eyeblinkEOGchannel; % EOG channel (name/idx) to plot
if isfield(options.eeg.preproc, 'eyeblinkEEGchannel')
    EEGchannel  = options.eeg.preproc.eyeblinkEEGchannel; % EEG channel (name/idx) to plot
else
    EEGchannel  = 'none';
end
nExcluded.all   = 0;
