function D = compi_spm_eeg_artefact(S)
% Simple artefact detection, optionally with robust averaging
% Code based on spm_eeg_artefact
% 
...
if isequal(S.mode, 'reject')
    ...
    for i = 1:numel(S.methods)
        ...
        if ~isempty(chanind)
            ...
            bad = bad | feval(['compi_spm_eeg_artefact_' S.methods(i).fun], S1);
        end
    end
...
elseif isequal(S.mode, 'mark')
    ...
    for i = 1:numel(S.methods)
        ...
        if ~isempty(chanind)
            ...
            D =  feval(['compi_spm_eeg_artefact_' S.methods(i).fun], S1);
        end
    end    
