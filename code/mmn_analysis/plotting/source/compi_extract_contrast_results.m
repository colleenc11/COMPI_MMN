function [ con ] = compi_extract_contrast_results( xSPM, titleStr )
...
iClus = 0;
 while numel(find(isfinite(statValue)))
     ...
     if clustAssign(maxIdx) == max(clustAssign)
         allClusterTimes    = XYZmmAll(1, [(end-(numVox-1)):1:end]);
     elseif clustAssign(maxIdx) == min(clustAssign)
         allClusterTimes    = XYZmmAll(1, [1:1:numVox]);
     elseif clustAssign(maxIdx) == 2
         numVoxMin          = numVoxels(1);
         allClusterTimes    = XYZmmAll(1, [(numVoxMin+1):1:(numVoxMin+numVox)]);
     end


%      allClusterTimes     = allClusterVoxels(3, :);
     clusterTimeWindow   = [peakCoords(1) peakCoords(1)];
