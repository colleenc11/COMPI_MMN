function D = compi_spm_eeg_spatial_confounds(S, doCompareAllTrials, details)
...
%              SPMEEG, CLEAR)
%
% The code, based on spm_eeg_spatial_confounds, has been modified as
% follows:
%   1. Originally, the code performed SVD analysis using data from all
%      trials, including bad ones.
%   2. We have changed the code so that it now exclusively uses good trials
%      for SVD analysis. This is done to ensure that the component primarily
%      represents trials with eyeblinks while excluding outlier bad trials.
%   3. If doCompareAllTrials is set to true, we additionally compare the
%      original SVD analysis (using all trials) with the new analysis 
%      (using only good trials). This allows us to assess the differences 
%      between the two analyses and ensure a more comprehensive capture of 
%      trials with eyeblinks.
%   4. In original code, the thresholding condition used the maximum
%      absolute value of 'temp' to check if it exceeded the specified
%      threshold 'S.threshold'. In the modified code, the 'abs' function
%      was removed from the thresholding condition, resulting in the
%      comparison of the maximum value (considering both positive and
%      negative values) of 'temp' against the threshold. 
...
switch upper(S.mode)
    ...
    case 'SVD'
        % Get the condition list from the input data structure D
        cl = D.condlist;
        % Initialize the svdinput matrix
        svdinput = [];

        % Loop through each condition in the condition list
        for i = 1:numel(cl)
            % Extract data from GOOD channels and trials within the specified time window
            tmp      = D(D.indchantype('MEEG', 'GOOD'), D.indsample(1e-3*S.timewin(1)):D.indsample(1e-3*S.timewin(2)), D.indtrial(cl{i},'GOOD'));
            % Reshape and concatenate the extracted data into svdinput
            svdinput = [svdinput, reshape(tmp, size(tmp, 1),  [])];
        end
        % Perform SVD on svdinput (created from using only good trials)
        [U, L, V] = spm_svd(svdinput);
        
        % Compare the original SVD analysis (using all trials) with the new
        % analysis (using only good trials)
        if doCompareAllTrials
            % Re-compute SVD using data from all trials
            for i = 1:numel(cl)
                tmp      = D(D.indchantype('MEEG', 'GOOD'), D.indsample(1e-3*S.timewin(1)):D.indsample(1e-3*S.timewin(2)), :);
                % Reshape and concatenate the extracted data into svdinput
                svdinput = [svdinput, reshape(tmp, size(tmp, 1),  [])];
            end
            % Perform SVD again on the updated svdinput (using all trials)
            [U2, L2, V2] = spm_svd(svdinput);
            
            %% Plot difference in SVD
            iComp=1:6;
            fh=figure('Name', 'Comparison removing bad eyeblinks to using all for confound components (U of SVD)');
            cm=hsv(numel(iComp)); % adapt colormap
            set(fh, 'DefaultAxesColorOrder', cm);
            hp = plot(U(:,iComp), '-'); hold on;
            plot(U2(:,iComp), ':'), plot(U(:,iComp)-U2(:,iComp), '--');

            % Legend for the plot, showing singular values relative to total variance
            legend(hp, num2str(diag(L(iComp,iComp))/sum(diag(L))));
            title({'First confound components ignoring bad eye blinks (-),', ...
                'using all trials (:) and difference(--); singular values relative to total variance plotted in legend'});

            % Diagnostics: Save the SVD components figure
            saveas(gcf,details.eeg.componentconfoundsfigure,'fig');
        end
        
        % Note, abs was removed from max(max(abs(temp)))>S.threshold
        ...
        % sconf calculated based on SVD analysis using only good trials
        if S.ncomp>0
