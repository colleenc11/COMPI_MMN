function [ con ] = tnueeg_extract_contrast_results( xSPM, titleStr )
...
iClus = 0;
 while numel(find(isfinite(statValue)))
     ...
     numVox              = numVoxels(maxIdx);

     if clustAssign(maxIdx) == max(clustAssign)
         allClusterTimes    = XYZmmAll(3, [(end-(numVox-1)):1:end]);
     elseif clustAssign(maxIdx) == min(clustAssign)
         allClusterTimes    = XYZmmAll(3, [1:1:numVox]);
     elseif clustAssign(maxIdx) == 2
         numVoxMin          = numVoxels(1);
         allClusterTimes    = XYZmmAll(3, [(numVoxMin+1):1:(numVoxMin+numVox)]);
     else
         allClusterTimes    = [peakCoords(3) peakCoords(3)];
     end


%      allClusterTimes     = allClusterVoxels(3, :);
     clusterTimeWindow   = [peakCoords(3) peakCoords(3)];
     clusterTimeWindowAll   = [min(allClusterTimes) max(allClusterTimes)];
...
%      clusterTimeWindowAll   = [min(allClusterTimes) max(allClusterTimes)];
     ...
     if isfield(con, 'clust') 
         ...
         if Pk < con.clust.threshold
             ...
             con.clusters(iClus).timewinall = clusterTimeWindowAll;
 ...
 % Remove any empty clusters that did not reach significance 
i_cluster = 1;
 while i_cluster < length(con.clusters)
     if isempty(con.clusters(i_cluster).peak)
         con.clusters(i_cluster) = [];
     else
