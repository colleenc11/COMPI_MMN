...
%--------------------------------------------------------------------------
% Computes the grandaverages of ERPs in one condition.
...
try
    ...
    for i_reg = 1:numel(factorNames)
            ...
            if strcmp(options.subjects.group_labels{i_group}, options.condition)
                ...
                for sub = 1: nSubjects
                    ...
                    switch options.eeg.erp.type
                        case {'phase', '2bins'}
                            erpfiles{sub, 1} = fullfile(details.eeg.erp.root, factorNames{i_reg}, [factorNames{i_reg} '.mat']);
                        otherwise
                            erpfiles{sub, 1} = fullfile(details.eeg.erp.root, factorNames{i_reg}, [factorNames{i_reg} '.mat']);
                    end
        ...
        for iCh = 1: numel(options.eeg.erp.channels)
            ...
            ga = tnueeg_grandmean_with_error(erpfiles, channel, factorNames{i_reg}, 1);
            save(fullfile(GAroot, [channel, '_ga']), 'ga');
            
            compi_grandmean_plot(ga, channel, factorNames{i_reg}, options, 1);
        end
        close all;
